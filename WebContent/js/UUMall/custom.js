...
var CustomMall = function() {
	...
	/*订单界面跳转*/
	orderLink = function() {
		if(this.attr('data-addr').indexOf("flag")!=-1){
//			window.location = this.attr('data-addr') + '&orderNo=' + (this.closest('.order-elem').attr('data-id') || $$('.order-num .num').text())
			$$.ajax({
				url : this.attr('data-addr') + '&orderNo=' + (this.closest('.order-elem').attr('data-id') || $$('.order-num .num').text()),
				method : "GET",
				cache : false,
				dataType : "json",
...
==================================================*/
	...
	fn.orderConfirm = function() {
		...
		countPayNum = function() {
			...
			privi = function(num) {
				var balanceMoney=$(".toolbar-inner").find("span").eq(3).text();//余额
				var temp = num;
				...
				if (temp >= parseInt(arr[0])) {
					//使用优惠券
					$(".toolbar-inner").find("span").eq(3).text('￥0.00');
					$("._fm_ext_1").removeClass("active2");
					if(temp>=parseInt(arr[1])){
			...
			var temPri=$(".coupon-elem.do._fm-rel.active").attr("data-num");
			if(typeof(temPri)!="undefined"){
				price.eq(1).html('￥' + Number(temPri.split("-")[1]).toFixed(2));
			}else{
				price.eq(1).html('￥0.00');
			}
//			price.eq(1).html('￥' + ( oldNum - newNum ).toFixed(2));
