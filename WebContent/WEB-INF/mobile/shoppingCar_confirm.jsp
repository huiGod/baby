        ...
        <div class="view view-main _js-orderConfirm">
            <div class="navbar">
                ...
                <div class="navbar-inner cached _fm-block _fm-txtcenter _fm-link-1" data-page="editAddress">
                    ...
                    <a href="#" class="left back left-txt _fm-abs _fm-tz _fm-lz _fm-f-14"><i class="_fm-back-left">&nbsp;</i>返回</a>
                    <span class="right right-txt do _fm-abs _fm-tz _fm-rz _fm-f-14" data-op="delete" data-addr="<%=path%>/goods/deleteAdress.do">删除</span>
                </div>
            ...
            <div class="pages navbar-through toolbar-through">
                                    ...
                                    <div class="order-msg">
                                       ...
                                       <c:if test="${isOne==1}">
                                        ...
                                        <p class="one-item edit-num _fm-f-13 _js-formatminus">
                                            ...
                                            <span class="_fm-right num numValue">${goods.num}</span>
                                       ...
                                       <!-- 单件商品/END -->
	                                    ...
	                                    <p class="division _fm-rel"></p>
                                        <div class="remarkInfo one-item _fm-f-13">
	                                          <h3>91余额：${balance}元<span class="_fm-check-1 _fm_ext_1">&nbsp;</span></h3>
	                                    </div>
                                        <c:if test="${isOne==1}">
                                         <!--  <span class="submit do _fm-right _fm-txtcenter _fm-f-18" data-addr="<%=path%>/UOrder/formOrderOne.do" data-op="submit">提交订单</span> -->
                                          <form method='post'  class="_fm-hidden main-form" action="<%=path%>/goods/formOrderOne.do">
                                        </c:if>
                                        <c:if test="${isOne==2}">
                                          ...
                                          <form method='post'  class="_fm-hidden main-form" action="<%=path%>/goods/createOrderByShops.do">
                                        </c:if>
                ...
                <div class="page cached no-toolbar" data-page="editAddress">
                    ...
                    <div class="save-use disabled do _fm-f-14 _fm-abs _fm-bz _fm-lz _fm-txtcenter" data-op="saveAndUse" data-addr="<%=path%>/goods/updateAdress.do">保存并使用</div>
                </div>
                            ...
                            <div class="cus-page-inner">
                                ...
                                <div class="main-content coupon-list">
                                  <c:forEach items="${coupon}" var="c">
                                    <section class="coupon-elem do _fm-rel" data-num="${c.orgPrice}-${c.nowPrice}" data-op="useCoupon" data-id="${c.id}">
                                        <div class="deno _fm-abs _fm-img _fm-tz _fm-lz">
                                            <div class="img-inner">
                                                <span class="num _fm-block">${c.nowPrice}<span class="_fm-f-16">元</span></span>
                                                <span class="_fm-block"><c:if test="${c.type==1}">优惠券</c:if><c:if test="${c.type==2 }">代金券</c:if></span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="elem-inner _fm-rel">
                                            ...
                                            <h1 class="_fm-f-13 _fm-ellipsis">91特惠商品现金优惠券</h1>
                                            <p class="_fm-overhide">${c.about}<c:if test="${c.type==1}">(需要满${c.orgPrice}元)</c:if></p>
                                            <div><c:if test="${c.type==1}">有效期至${c.valid}</c:if></div>
                                        </div>
                                    </section>
                                  </c:forEach>
            ...
            <div class="toolbar _fm-f-18">
                <div class="toolbar-inner _fm-block _fm-rel _fm-overhide">
                    <div class="_fm-left">
                        ...
                        <p class="detail _fm-f-12">已优惠:<span>￥0.00</span>已减免:<span id="yimianjian">￥0.00</span></p>
                    </div>
                    <c:if test="${isOne==1}">
                      <span class="submit do _fm-right _fm-txtcenter _fm-f-18" id="submit1">提交订单</span>
                    </c:if>
                    <c:if test="${isOne==2}">
                      <span class="submit do _fm-right _fm-txtcenter _fm-f-18" id="submit2">提交订单</span>
                    </c:if>
                </div>
            </div>
        </div>
...
==================================================-->
	...
	$(document).ready(function(){
    	$("._fm_ext_1").click(function(){
    		var money=${balance};
    		var flag=$(this).hasClass("active2");
    		var allPriceObject=$(".toolbar-inner").find("span").eq(0);
    		var allPriceMoney=allPriceObject.text().substring(1);
    		if(!flag){
    			$(this).addClass("active2");
    			if(Number(money)>Number(allPriceMoney)){
    				$(".detail._fm-f-12").find("span").last().text('￥'+Number(allPriceMoney).toFixed(2));
    				allPriceObject.html('￥<span style="font-size: 18px">0.</span>00');
    			}else{
    				$(".detail._fm-f-12").find("span").last().text('￥'+Number(money).toFixed(2));
    				var allMoney=String((Number(allPriceMoney)-Number(money)).toFixed(2));
    				allPriceObject.html('￥<span style="font-size: 18px">'+allMoney.split(".")[0]+'.</span>'+allMoney.split(".")[1]);
    			}
    		}else{
    			$(this).removeClass("active2");
    			var mianjian=$(".detail._fm-f-12").find("span").last().text().substring(1);
    			var resuMoney=(Number(allPriceMoney)+Number(mianjian)).toFixed(2);
    			allPriceObject.html('￥<span style="font-size: 18px">'+String(resuMoney).split(".")[0]+'.</span>'+String(resuMoney).split(".")[1]+'');
    			$(".detail._fm-f-12").find("span").last().text('￥0.00');
    		}
    	});
    	$("#submit1").click(function(){
    		var orderNo=$("input[name=orderNo]").val();
    		var couponId=$(".coupon-elem.active").attr("data-id");
    		var addressId=$(".order-address.active").attr("data-id");
    		var num=$(".numValue").text();
    		var payId="1";
    		...
    		$.ajax({
        		url: "/baby/goods/formOrderOne.do",
        		data:{
        			orderNo:orderNo,
        			couponId:couponId,
        			addressId:addressId,
        			...
        			payId:payId,
        			remark:remark,
        			balance:$("#yimianjian").text().substring(1)
        		},
                type: 'POST',
                dataType: 'json',
                cache:false,
                ...
                success:function(data){
                    if(data.code==200){
                    	window.location="pay://?orderNo="+data.data.orderNo+"&totalFee="+data.data.totalFee+"&creatTime="+data.data.creatTime;
    	...
    	$("#submit2").click(function(){
    		var shopNos=$("input[name=shopNos]").val();
    		var couponId=$(".coupon-elem.active").attr("data-id");
    		var addressId=$(".order-address.active").attr("data-id");
    		var payId="1";
    		var remark=$(".remarkTextarea").val();
    		...
    		$.ajax({
        		url: "/baby/goods/createOrderByShops.do",
        		data:{
        			shopNos:shopNos,
        			couponId:couponId,
        			addressId:addressId,
